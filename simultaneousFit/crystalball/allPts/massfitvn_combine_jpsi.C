      ...
      )
{
   ...
   gStyle->SetOptStat(0);
    double fit_range_low = 2.55;
    double fit_range_high = 3.65;
...
//        TF1* f1 = new TF1(Form("f_sig_%d",i),"[0]*([5]*([4]*TMath::Gaus(x,[1],[2]*(1.0 +[6]))/(sqrt(2*3.14159)*[2]*(1.0 +[6]))+(1-[4])*TMath::Gaus(x,[1],[3]*(1.0 +[6]))/(sqrt(2*3.14159)*[3]*(1.0 +[6]))))", fit_range_low, fit_range_high);
        ...
        for(int k=0;k<vn_data->GetN();k++){
          vn_data->SetPointError(k,0,vn_data->GetErrorY(k)*1); //Check the error of v2
        }
...
//        leg1->AddEntry(f2,"K-#pi swap","f");
 ...
 histv2->SetMinimum(-0.0199);
 histv2->SetMaximum(0.0601);
 ...
 v2plot->Draw("psameez");
    ...
    TLatex *tex2= new TLatex(2.05,0.05,"pp 13 TeV, 100 #leq N_{trk}^{offline}");
    tex2->SetTextColor(1);
    tex2->SetTextSize(0.05);
    tex2->SetTextFont(42);
    tex2->Draw();
    //TLatex *tex3= new TLatex(2.05,0.10,"1.4 < |y| < 2.4");
    TLatex *tex3= new TLatex(2.05,0.041,"1.4 < |y| < 2.4");
    tex3->SetTextColor(1);
